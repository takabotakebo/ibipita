...
(function(Sushi) {
	...
	function initWebCL(web_cl, env, $M) {
		...
		switch(env) {
			case 'node':
			case 'ff':
				$CL.queue = $CL.context.createCommandQueue($CL.devices[device_index], 0);
				break;
			case 'chromium':
			...
			for (var i = 0; i < params.length; i++) {
				if (params[i].type === void 0) {
					...
					if (!params[i].datum.buffer) {
						params[i].datum.buffer = $CL.context.createBuffer(web_cl.MEM_READ_WRITE, params[i].datum.byte_length);
						$CL.buffers++;
